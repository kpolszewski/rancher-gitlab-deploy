...
def main(rancher_url, rancher_key, rancher_secret, cluster, environment, stack_name, service, new_image, upgrade_timeout, wait_for_upgrade_to_finish, debug):
	...
	try:
		r = session.get("%s/clusters?limit=-1" % api)
		r.raise_for_status()
	...
	try:
		r = session.get("%s/projects?limit=-1&all=true" % api)
		r.raise_for_status()
	except requests.exceptions.HTTPError:
		bail("Unable to connect to Rancher at %s - is the URL and API key right?" % host)
	else:
	...
	try:
		r = session.get("%s/projects/%s/stacks?limit=-1" % (
			api,
			environment_id
		))
		r.raise_for_status()
	...
	try:
		r = session.get("%s/projects/%s/stacks/%s/services?limit=-1" % (
			api,
			environment_id,
			stack['id']
		))
	...
	if new_image:
		upgrade['inServiceStrategy']['launchConfig']['image'] = '%s' % new_image
